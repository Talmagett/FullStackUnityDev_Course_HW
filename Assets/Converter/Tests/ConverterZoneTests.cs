...
namespace Converter.Tests
{
    public class ConverterZoneTests
    {
        ...
        [TestCase(5, 5, 3, 2, 3)]
        [TestCase(5, 5, 7, 0, 5)]
        public void RemoveResourcesFromZone(int limit, int addingCount, int removingCount, int expectedCount,
        ...
        [TestCase(5, 5, 0, 5)]
        public void RemoveResourcesFromZoneWithOutOfRangeException(int limit, int addingCount, int removingCount,
            int expectedCount)
        {
            var converterZone = new ConverterZone(limit);
            converterZone.PutResources(addingCount);
            Assert.Catch<ArgumentOutOfRangeException>(() => { converterZone.RemoveResources(removingCount); });
        }
    }
}